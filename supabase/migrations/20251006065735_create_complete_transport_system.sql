/*
  # Système de transport complet pour le Burundi avec prix
  ...
  ### 2. bus_lines (Lignes de bus urbains)
    - `id` (uuid, primary key)
    - `name` (text) - Nom de la ligne
    - `city_id` (uuid) - Référence à la ville
    - `zones_covered` (text[]) - Zones traversées
    - `route_coordinates` (jsonb) - Tracé GPS
    - `stops` (jsonb) - Liste des arrêts
    - `color` (text) - Couleur pour la carte
    - `price` (integer) - Prix du trajet en FBU
    - `active` (boolean) - Ligne active
    - `created_at` (timestamptz)

  ### 3. transport_agencies (Agences de transport)
    - `id` (uuid, primary key)
  ...
  ### 4. intercity_routes (Trajets interprovinciaux)
    ...
    - `route_coordinates` (jsonb) - Tracé GPS
    - `departure_point` (text) - Point de départ
    - `arrival_point` (text) - Point d'arrivée
    - `frequency` (text) - Fréquence des départs
    - `duration_minutes` (integer) - Durée estimée
    - `price` (integer) - Prix en FBU
    - `schedule` (jsonb) - Horaires
    - `active` (boolean)
    - `created_at` (timestamptz)
...
CREATE POLICY "Authenticated users can manage bus lines"
...
CREATE POLICY "Authenticated users can manage intercity routes"
  ON intercity_routes FOR ALL
  TO authenticated
  USING (true)
  WITH CHECK (true);